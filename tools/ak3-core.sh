...
patch_kernelsu() {
 cd $home;
 $bin/ksud boot-patch -b $bootimg -m $home/kernelsu.ko --magiskboot $bin/magiskboot
  mv 
 if [ -f $home/kernelsu_patched*.img ]; then
    mv $home/kernelsu_patched*.img $home/boot-new.img
  else
    abort "No kernelsu patched file found to flash. Aborting...";
  fi;
  if [ "$(wc -c < boot-new.img)" -gt "$(wc -c < boot.img)" ]; then
    abort "New image larger than target partition. Aborting...";
  fi;
  blockdev --setrw $block 2>/dev/null;
  if [ -f "$bin/flash_erase" -a -f "$bin/nandwrite" ]; then
